...
```{r}
setwd("C:/Users/Ghislaine/Desktop/meta_analysis/feedback/code/code_meta/data")
file_path="C:/Users/Ghislaine/Desktop/meta_analysis/feedback/code/code_meta/data"
```
...
# Define the updated plot theme without grid lines
plot_theme <- theme_minimal(base_size = 30) +  # Increase base font size
  theme(
    plot.title = element_text(face = "bold", size = 26, color = "black"),  # Increase title size
    plot.subtitle = element_text(face = "plain", size = 22, color = "grey70"),  # Increase subtitle size
    legend.title = element_text(face = "bold", size = 20, color = "black"),  # Increase legend title size
    legend.text = element_text(size = 22),  # Increase legend text size
    axis.title = element_text(face = "bold", size = 40, color = "black"),  # Increase x and y axis title size
    axis.text = element_text(size = 40, color = "black"),  # Increase x and y axis tick labels size
    ...
    legend.box.background = element_rect(color = "grey80"),  # Light grey border around legend
    legend.box.margin = margin(5,5,5,5),
    legend.key.width = unit(1.5, "cm")
  )


# ### plot theme for the non distribution plots
# plot_theme <- theme_minimal(base_size = 20) +  # Increase base font size
#   theme(
#     plot.title = element_text(face = "bold", size = 26, color = "black"),  # Increase title size
#     plot.subtitle = element_text(face = "plain", size = 22, color = "grey70"),  # Increase subtitle size
#     legend.title = element_text(face = "bold", size = 20, color = "black"),  # Increase legend title size
#     legend.text = element_text(size = 18),  # Increase legend text size
#     axis.title = element_text(face = "bold", size = 24, color = "black"),  # Increase x and y axis title size
#     axis.text = element_text(size = 22, color = "black"),  # Increase x and y axis tick labels size
#     panel.grid.major.x = element_blank(),  # Remove major grid lines along x
#     panel.grid.major.y = element_blank(),  # Remove major grid lines along y
#     panel.grid.minor.x = element_blank(),  # Remove minor grid lines along x
#     panel.grid.minor.y = element_blank(),  # Remove minor grid lines along y
...
plot_data <- plot_data %>%
    ...
    if (is_continuous) {
      ...
      plot <- ggplot(plot_data, aes(x = hedges_g, y = .data[[moderator]])) +
        ...
        annotate("text", x = max(plot_data$hedges_g, na.rm = TRUE) / 2, 
                 y = min_y - 1, 
                 label = "Immediate Feedback", hjust = 0.5, color = "#893F45", size=12)  +
        annotate("text", x = min(plot_data$hedges_g, na.rm = TRUE) / 2, 
                 y = min(plot_data[[moderator]], na.rm = TRUE) - 1, 
                 label = "Delayed Feedback", hjust = 0.5, color = "#893F45",size=12)+  
        plot_theme  
        # theme(
  ...
  plot <- ggplot(plot_data, aes(x = .data[[moderator]], y = hedges_g)) +
    ...
    geom_segment(aes(x = 0.55, y = 0, xend = 0.55, yend = -1.5), 
                 arrow = arrow(length = unit(0.2, "cm")), color = "#893F45", size=1.5) +
    geom_segment(aes(x = 0.55, y = 0, xend = 0.55, yend = 1.5), 
                 arrow = arrow(length = unit(0.2, "cm")), color = "#893F45",size=1.5) +
    # Labels for arrows, repositioned for clarity
    annotate("text", x = 0.5, y = -0.8, label = "Delayed Feedback", hjust = 0.5, color = "#893F45", size=11) +
    annotate("text", x = 0.5, y = 0.8, label = "Immediate Feedback", hjust = 0.5, color = "#893F45",size=11) +
    # Annotate the number of points for each group
    geom_text(data = n_points_per_group, 
              aes(x = .data[[moderator]], 
                  y = max(plot_data$hedges_g, na.rm = TRUE) , 
                  label = paste0("n = ", n)), 
              vjust = -1, 
              color = "#525252", 
              fontface = "bold", 
    ...
    if (common_plot == TRUE) {
      ...
      ggsave(filename = paste0(name, "_combined_plot.png"), plot = plot, width = 15, height = 10)
      return(plot)
    } else {
      names(plot_list) <- moderators
      for (i in seq_along(plot_list)) {
        ggsave(filename = paste0(name, "_", moderators[i], ".png"), plot = plot_list[[i]], width = 16, height = 15)
      }
      return(plot_list) 
    }
...
moderator_plots_list=moderator_plots(for_moderator_plots, "", moderators_list=preregistered_moderators_plot_list, threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=FALSE)
...
```{r}
plot_descriptive_nb_effect_size=plot_distribution(data = data_clean, variable = "time_limit_for_answer", variable_type = "categorical", grouping_value = "effects")
plot_descriptive_nb_effect_size
ggsave("descriptive.png", plot = plot_descriptive_nb_effect_size, width = 8, height = 6, dpi = 300)
```
...
for_moderator_plots <- data_clean %>%
  ...
  rename(
    ...
    'log(delay_difference)' = feedback_delay_difference_converted_to_time_log,
...
moderator_plots_list_exploratory=moderator_plots(for_moderator_plots_exploratory, "", moderators_list=exploratory_moderators_plot_list, threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=FALSE)

moderator_plots_list_exploratory
```

### for continious variables the moderator plots

```{r}
moderator_plots_for_continious <- function(data_input, name, threshold_for_moderator = threshold_for_moderator, moderators_list = NULL, common_plot = TRUE, remove_small_groups = TRUE) {
...
plot_data <- plot_data %>%
  mutate(across(all_of(moderator), ~ gsub("_", " ", .)))
    ...
    if (is_continuous) {
      ...
      # Calculate total number of points for annotation
      n_points <- nrow(plot_data)
            # Calculate the one unit offset from the minimum x-value
offset <- 4  # Adjust this offset as needed based on your data scale or preference
# Define dynamic position adjustment based on the value of max(moderator)
# Define dynamic position adjustment based on the value of max(moderator)
x_position <- ifelse(max(plot_data[[moderator]], na.rm = TRUE) > 500, 
                     max(plot_data[[moderator]], na.rm = TRUE) - offset,  # If > 500, use offset
                     ifelse(max(plot_data[[moderator]], na.rm = TRUE) > 100, 
                            max(plot_data[[moderator]], na.rm = TRUE) - 50,  # If > 100, subtract 50
                            ifelse(max(plot_data[[moderator]], na.rm = TRUE) > 50, 
                                   max(plot_data[[moderator]], na.rm = TRUE) - 10,  # If between 50 and 100, subtract 10
                                   max(plot_data[[moderator]], na.rm = TRUE) - offset)))  # Default case

      # Plot for continuous moderator
      plot <- ggplot(plot_data, aes(x = .data[[moderator]], y = hedges_g)) +
  geom_point(aes(color = se_g), alpha = 0.6, size=4) +
  tryCatch({
    geom_smooth(method = "lm", se = TRUE, color = "darkred")
  }, error = function(e) {
    message("Error in geom_smooth for moderator: ", moderator)
  ...
  # Annotation with total number of points and total NAs
  annotate("text", x = x_position, 
           ...
           fontface = "bold", 
           size = 14) +
    labs(x = gsub("_", " ", moderator), 
       y = paste0("Hedge's g (", name, ")"),
       color = "SE of Hedges' g") +  # Updated legend title
  # geom_segment(aes(x = min(plot_data[[moderator]], na.rm = TRUE), y = 0, 
  #                  xend = min(plot_data[[moderator]], na.rm = TRUE), yend = max(plot_data$hedges_g, na.rm = TRUE)), 
  #              arrow = arrow(length = unit(0.2, "cm")), color = "#893F45") +
  # geom_segment(aes(x = min(plot_data[[moderator]], na.rm = TRUE), y = 0, 
  #                  xend = min(plot_data[[moderator]], na.rm = TRUE), yend = min(plot_data$hedges_g, na.rm = TRUE)), 
  #              arrow = arrow(length = unit(0.2, "cm")), color = "#893F45") +
  # annotate("text", x = min(plot_data[[moderator]], na.rm = TRUE) + offset, 
  #          y = max(plot_data$hedges_g, na.rm = TRUE) / 2, 
  #          label = "Immediate Feedback", hjust = 0, color = "#893F45") +
  # annotate("text", x = min(plot_data[[moderator]], na.rm = TRUE) + offset, 
  #          y = min(plot_data$hedges_g, na.rm = TRUE) / 2, 
  #          label = "Delayed Feedback", hjust = 0, color = "#893F45") +
  plot_theme + 
  xlim(min(plot_data[[moderator]], na.rm = TRUE), max(plot_data[[moderator]]+1, na.rm = TRUE)) # Set x-axis limits
  ...
  plot <- ggplot(plot_data, aes(x = .data[[moderator]], y = hedges_g)) +
    ...
    geom_segment(aes(x = 0.55, y = 0, xend = 0.55, yend = 1.5), 
                 arrow = arrow(length = unit(0.2, "cm")), color = "#893F45") +
    # Labels for arrows, repositioned for clarity
    annotate("text", x = 0.5, y = -0.8, label = "Delayed Feedback", hjust = 0.5, color = "#893F45", size=8) +
    annotate("text", x = 0.5, y = 0.8, label = "Immediate Feedback", hjust = 0.5, color = "#893F45",size=8) +
    # Annotate the number of points for each group
    geom_text(data = n_points_per_group, 
              aes(x = .data[[moderator]], 
                  y = max(plot_data$hedges_g, na.rm = TRUE) , 
...
for_moderator_plots_continious <- data_clean %>%
  select(hedges_g,
    se_g,
    feedback_delay_difference_converted_to_time_log,
    nb_item_in_training, 
...
continious_moderators_plot_list= c(
  "log(delay_difference)",
  "nb_training_items", 
  "publication_year",
  "retention_interval(day)"
...
moderator_plots_list_continious=moderator_plots_for_continious(for_moderator_plots_continious, "", moderators_list=continious_moderators_plot_list, threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=FALSE)

moderator_plots_list_continious
...
moderator_plots_list=moderator_plots(data_clean, "", moderators_list="publication_year", threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=TRUE)
...
mean(item_data$feedback_delay_difference, na.rm = TRUE)
min(item_data$feedback_delay_difference, na.rm = TRUE)
max(item_data$feedback_delay_difference, na.rm = TRUE)
sd(item_data$feedback_delay_difference, na.rm = TRUE)
...
```
```{r}
moderator_plots_list=moderator_plots(item_data, "", moderators_list=preregistered_moderators_for_item, threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=FALSE)

moderator_plots_list
```
```{r}

item_data_mod_plot <- item_data %>%
  rename(
    'delay_difference(nb_items)' = feedback_delay_difference
  )

moderator_plot_only_item=moderator_plots_for_continious(item_data_mod_plot, "", moderators_list="delay_difference(nb_items)", threshold_for_moderator= 2, common_plot = FALSE, remove_small_groups=FALSE)

moderator_plot_only_item
```
...
data_selected <- data_clean[, c("unique_ID", 
                                "expt_num",  
                                "feedback_type_simple_elaborated",  
                                "delay_definition",  
                                "feedback_delay_difference",  
...
tex_filename <- normalizePath(file.path(file_path, "feedback_analysis_table.tex"), mustWork = FALSE)

# Create the LaTeX table
latex_table <- data_selected %>%
  kbl(
    format = "latex",
    booktabs = TRUE,
    align = "lrrrrrrrrr"  # Adjust column alignment as needed
  ) %>%
  kable_styling(
    latex_options = c("hold_position"),
    full_width = FALSE
  ) %>%
  add_header_above(setNames(ncol(data_selected), "Feedback Meta-Analysis Summary"))  # Adjust title
...
tryCatch({
  ...
  warning("An error occurred while writing the file: ", e$message)
})
